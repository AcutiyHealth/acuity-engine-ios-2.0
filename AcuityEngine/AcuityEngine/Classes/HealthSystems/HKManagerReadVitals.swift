                        ...
                        if error == nil {
                            ...
                            for preferredUnit in preferredUnits {
                                ...
                                do {
                                    ...
                                    let query = try self.reporter?.reader.statisticsCollectionQuery(type: preferredUnit.identifier, unit: preferredUnit.unit, anchorDate: anchorDate, enumerateFrom: startOfDaysAgo, enumerateTo: now, intervalComponents: interval, resultsHandler: { [weak self] (results, error) in
                                        if error == nil{
                                            results?.enumerateStatistics(from: startOfDaysAgo, to: now){ (data, stop) in
                                                do {
                                                    let unit = HKUnit.init(from: preferredUnit.unit)
                                                    //totalCount = totalCount+1
                                                    //print("totalCount",totalCount,data.startDate,data.quantityType)
                                                    let statistics = try? Statistics.init(statistics: data, unit: unit)
                                                    
                                                    let identifier =  try? QuantityType.make(from: preferredUnit.identifier)
                                                    if statistics != nil,identifier != nil,let averageValue = statistics?.harmonized.average{
                                                        let value = averageValue
                                                        let unitStringForVital = statistics!.harmonized.unit
                                                        //print("statistics--%@, averageValue----",statistics?.identifier as Any,averageValue)
                                                        //save data For Cardio
                                                        CardioManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp)
                                                        
                                                        //save data For Respiratory
                                                        RespiratoryManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp)
                                                        
                                                        //save data For Renal
                                                        RenalManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp)
                                                        
                                                        //Save data for ID...
                                                        IDiseaseManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp, unit: unitStringForVital)
                                                        
                                                        //Save data for FNE...
                                                        FNEManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp)
                                                        
                                                        //Save data for Hemato...
                                                        HematoManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp, unit: unitStringForVital)
                                                        
                                                        //Save data for Endocrine...
                                                        EndocrineManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp, unit: unitStringForVital)
                                                        
                                                        //Save data for Gastrointestinal...
                                                        GastrointestinalManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp)
                                                        
                                                        //Save data for Genitourinary...
                                                        GenitourinaryManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp, unit: unitStringForVital)
                                                        
                                                        //Save data for Neuro System...
                                                        NeuroManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp)
                                                        
                                                        //Save data for SDH System...
                                                        SDHManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp)
                                                        
                                                        //Save data for Musc System...
                                                        MuscManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp)
                                                        
                                                        //Save data for Skin System...
                                                        SkinManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp, unit: unitStringForVital)
                                                        
                                                        //Save data for Heent System...
                                                        HeentManager.sharedManager.saveQuantityInArray(quantityType: identifier!, value: value, startTimestamp: statistics!.startTimestamp, unit: unitStringForVital)
                                                    }
                                                }
                                            }
                                        }
            ...
            reporter.manager.requestAuthorization(
                ...
                if success && error == nil {
                    ...
                    reporter.manager.preferredUnits(for: types) { (preferredUnits, error) in
                        ...
                        if error == nil {
                            //print("timeStamp start:",Date().timeIntervalSince1970)
                            let now = Date()
                            let component = Calendar.Component.month
                            let beforeDaysOrWeekOrMonth = 3
                            let daysAgo = Calendar.current.date(byAdding: component, value: -beforeDaysOrWeekOrMonth, to: now)!
                            ...
                            dispatchGroup.notify(queue: .main) {
                                ...
                                DispatchQueue.main.async {
                                    //print("timeStamp end:",Date().timeIntervalSince1970)
